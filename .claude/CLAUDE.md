...
`build.gradle` 使用 **ext** 管理版本号（非 `libs.versions.toml`）。`taotao-cloud-dependencies:2026.11` 为私有 BOM，外部构建需私有仓库凭据。
...
- **私有 BOM**：`taotao-cloud-dependencies:2026.11` 需私有仓库凭据，外部构建无法直接运行
